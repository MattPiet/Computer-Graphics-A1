...
void main() {
    ...
    mat3 normalMatrix = mat3(transpose(inverse(planeModelMatrix)));
    vertNormal = normalize(normalMatrix * vNormal); /// Rotate the normal to the correct orientation 
    vec3 vertPos = vec3(viewMatrix * planeModelMatrix * vVertex);
    vec3 vertDir = normalize(vertPos);
    eyeDir = -vertDir;
   for(int i = 0; i < 5; i++){
    lightDir[i] = normalize(vec3(lightPosition[i]) - vertPos); 
    }
    gl_Position = projectionMatrix * viewMatrix * planeModelMatrix * vVertex;
}
...
//layout(location = 2) uniform mat4 planeModelMatrix;
